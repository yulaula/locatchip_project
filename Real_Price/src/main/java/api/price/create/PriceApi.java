...
public class PriceApi {
	...
	public void priceInform(String goodInspectDay) throws IOException {

		//goodInspectDay는 매주 금요일, 가장 가까운 금요일의 데이터는 올라오지 않았을 수 있음

		PriceUpdateDB pricedao = new PriceUpdateDB(JDBCTemplate.getConnection());
		List<Product> prodlist = new ProductApi().productInform();
		List<Price> pricelist = new ArrayList<Price>();
		List<String> goodIdList = new ArrayList<String>();

		for(int i = 0; i<prodlist.size() ; i++) {
			goodIdList.add(prodlist.get(i).getGoodId());
		}
		...
		for(String goodId : goodIdList) {
			...
			urlBuilder.append("&" + URLEncoder.encode("goodInspectDay","UTF-8") + "=" + URLEncoder.encode(goodInspectDay, "UTF-8"));
	...
	//		new PriceApi().priceInform("20211119", "1669", "");
	//	}
}
