<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ include file="../header.jsp"%>

<%
	
%>

<!-- Page Title-->
<div class="page-title-overlap bg-purple pt-4">
	<div class="container d-lg-flex justify-content-between py-2 py-lg-3">
		<div class="order-lg-1 pe-lg-4 text-center text-lg-start">
			<h1 class="h3 text-light mb-0">후기 작성</h1>
		</div>
	</div>
</div>
<div class="container pb-5 mb-2 mb-md-4">
	<div class="row">
		<section class="col-lg-8">
			<!-- Steps-->
			<div class="steps steps-light pt-2 pb-3 mb-5">
				<a class="step-item active" href="shop-cart.html">
					<div class="step-progress">
						<span class="step-count">1</span>
					</div>
					<div class="step-label">
						<i class="ci-cart"></i>장바구니
					</div>
				</a>
				<a class="step-item active" href="checkout-payment.html">
					<div class="step-progress">
						<span class="step-count">2</span>
					</div>
					<div class="step-label">
						<i class="ci-card"></i>결제
					</div>
				</a> <a class="step-item active current" href="checkout-review.html">
					<div class="step-progress">
						<span class="step-count">3</span>
					</div>
					<div class="step-label">
						<i class="ci-check-circle"></i>후기
					</div>
				</a>
			</div>
			<!-- Order details-->
			<h2 class="h6 pt-1 pb-3 mb-3 border-bottom">후기 작성</h2>
			
			<!-- Item-->
			<div
				class="d-sm-flex justify-content-between my-4 pb-3 border-bottom">
				<div class="d-sm-flex text-center text-sm-start">
					<a class="d-inline-block flex-shrink-0 mx-auto me-sm-4"
						href="marketplace-single.html"><img src="2_img/머거본꿀땅콩.gif"
						width="160" alt="Product"></a>
					<div class="pt-2">
						<h3 class="product-title fs-base mb-2">
							<a href="marketplace-single.html">머거본 꿀땅콩(135g)</a>
						</h3>
						<div class="fs-sm">
							<span class="text-muted me-2"><i
								class="ci-home opacity-60 me-2"></i>이마트 역삼점</span>
						</div>
						<div class="fs-lg text-accent pt-2">
							2,500<small>원</small>
						</div>
					</div>
				</div>
				<div
					class="pt-2 pt-sm-0 ps-sm-3 mx-auto mx-sm-0 text-center text-sm-end"
					style="max-width: 9rem;">
					<p class="mb-0">
						<span class="text-muted fs-sm">수량:</span><span>&nbsp;1</span>
					</p>
					<!-- 작성 전 -->
					<a href="review-write.html">
						<button class="btn btn-link px-0" type="button">
							<i class="ci-edit me-2"></i><span class="fs-sm">작성하기</span>
						</button>
					</a>
					<!-- 작성 후 -->
					<!-- <button class="btn btn-link px-0" type="button" style="color: rgb(0, 70, 0);"><i class="ci-check-circle me-2"></i><span class="fs-sm">작성완료</span></button> -->
				</div>
			</div>

			<!-- Client details-->
			<div class="bg-secondary rounded-3 px-4 pt-4 pb-2">
				<div class="row">
					<div class="col-sm-6">
						<h4 class="h6">주문 정보</h4>
						<ul class="list-unstyled fs-sm">
							<li><span class="text-muted">주문자명:&nbsp;</span>홍길동</li>
							<li><span class="text-muted">주소:&nbsp;</span>서울시 강남구</li>
							<li><span class="text-muted">번호:&nbsp;</span>010-1234-1234</li>
						</ul>
					</div>
					<div class="col-sm-6">
						<h4 class="h6">결제 방식</h4>
						<ul class="list-unstyled fs-sm">
							<li><span class="text-muted">신용카드:&nbsp;</span>**** ****
								**** 5300</li>
						</ul>
					</div>
				</div>
			</div>
			<!-- Navigation (desktop)-->
			<div class="d-none d-lg-flex pt-4">
				<div class="w-50 pe-3">
					<a class="btn btn-secondary d-block w-100"
						href="total_product.html"><i
						class="ci-arrow-left mt-sm-0 me-1"></i><span
						class="d-none d-sm-inline">쇼핑 계속하기</span><span
						class="d-inline d-sm-none">Back</span></a>
				</div>
				<div class="w-50 ps-2">
					<a class="btn btn-primary d-block w-100" href="index.html"><span
						class="d-none d-sm-inline">메인으로</span><span
						class="d-inline d-sm-none"></span><i
						class="ci-arrow-right mt-sm-0 ms-1"></i></a>
				</div>
			</div>
		</section>
		<!-- Sidebar-->
		<aside class="col-lg-4 pt-4 pt-lg-0 ps-xl-5">
			<div class="bg-white rounded-3 shadow-lg p-4 ms-lg-auto">
				<div class="py-2 px-xl-2">
					<h2 class="h6 text-center mb-4">주문 요약</h2>
					<ul class="list-unstyled fs-sm pb-2 border-bottom">
						<li class="d-flex justify-content-between align-items-center"><span
							class="me-2">픽업 요청사항 : </span><span class="text-end">이따
								찾으러 갑니다.</span></li>
					</ul>
					<h3 class="fw-normal text-center my-4">
						150,000<small>원</small>
					</h3>
					<form class="needs-validation" method="post" novalidate></form>
				</div>
			</div>
		</aside>
	</div>
	<!-- Navigation (mobile)-->
	<div class="row d-lg-none">
		<div class="col-lg-8">
			<div class="d-flex pt-4 mt-3">
				<div class="w-50 pe-3">
					<a class="btn btn-secondary d-block w-100"
						href="checkout-payment.html"><i
						class="ci-arrow-left mt-sm-0 me-1"></i><span
						class="d-none d-sm-inline">Back to Payment</span><span
						class="d-inline d-sm-none">Back</span></a>
				</div>
				<div class="w-50 ps-2">
					<a class="btn btn-primary d-block w-100"
						href="checkout-complete.html"><span class="d-none d-sm-inline">Complete
							order</span><span class="d-inline d-sm-none">Complete</span><i
						class="ci-arrow-right mt-sm-0 ms-1"></i></a>
				</div>
			</div>
		</div>
	</div>
</div>

<%@ include file="../footer.jsp"%>